// GENERATED by tools/import_companion.py from MeshCore's examples/companion_radio/MyMesh.cpp
...
void CompanionMesh::writeErrFrame(uint8_t err_code) {
  uint8_t buf[2];
  buf[0] = RESP_CODE_ERR;
  buf[1] = err_code;
...
void CompanionMesh::writeDisabledFrame() {
  uint8_t buf[1];
  buf[0] = RESP_CODE_DISABLED;
  _serial->writeFrame(buf, 1);
}

void CompanionMesh::writeContactRespFrame(uint8_t code, const ContactInfo &contact) {
  int i = 0;
  out_frame[i++] = code;
...
void CompanionMesh::updateContactFromFrame(ContactInfo &contact, uint32_t& last_mod, const uint8_t *frame, int len) {
  int i = 0;
...
bool CompanionMesh::Frame::isChannelMsg() const {
  return buf[0] == RESP_CODE_CHANNEL_MSG_RECV || buf[0] == RESP_CODE_CHANNEL_MSG_RECV_V3 ||
         buf[0] == RESP_CODE_CHANNEL_DATA_RECV;
}

void CompanionMesh::addToOfflineQueue(const uint8_t frame[], int len) {
...
int CompanionMesh::getFromOfflineQueue(uint8_t frame[]) {
...
float CompanionMesh::getAirtimeBudgetFactor() const {
  return _prefs.airtime_factor;
}

int CompanionMesh::getInterferenceThreshold() const {
  return 0; // disabled for now, until currentRSSI() problem is resolved
}
bool CompanionMesh::getCADEnabled() const {
  return false; // hardware CAD before TX (disabled by default, until configurable)
}

int CompanionMesh::calcRxDelay(float score, uint32_t air_time) const {
...
#ifdef DISPLAY_CLASS
    if (_ui) _ui->notify(CompanionUIEvent::newContactMessage);
#endif
  }

  // add inbound-path to mem cache
  if (path && mesh::Packet::isValidPathLen(path_len)) {  // check path is valid
    CompanionAdvertPath* p = advert_paths;
    uint32_t oldest = 0xFFFFFFFF;
    for (int i = 0; i < ADVERT_PATH_TABLE_SIZE; i++) {   // check if already in table, otherwise evict oldest
      if (memcmp(advert_paths[i].pubkey_prefix, contact.id.pub_key, sizeof(CompanionAdvertPath::pubkey_prefix)) == 0) {
...
static int sort_by_recent(const void *a, const void *b) {
  return ((CompanionAdvertPath *) b)->recv_timestamp - ((CompanionAdvertPath *) a)->recv_timestamp;
}

int CompanionMesh::getRecentlyHeard(CompanionAdvertPath dest[], int max_num) {
...
void CompanionMesh::queueMessage(const ContactInfo &from, uint8_t txt_type, mesh::Packet *pkt,
  ...
  if (should_display && _ui) {
    ...
    if (!_serial->isConnected()) {
      _ui->notify(CompanionUIEvent::contactMessage);
    }
  }
#endif
}

bool CompanionMesh::filterRecvFloodPacket(mesh::Packet* packet) {
...
void CompanionMesh::onMessageRecv(const ContactInfo &from, mesh::Packet *pkt, uint32_t sender_timestamp,
                           const char *text) {
  markConnectionActive(from); // in case this is from a server, and we have a connection
  queueMessage(from, TXT_TYPE_PLAIN, pkt, sender_timestamp, NULL, 0, text);
}

void CompanionMesh::onCommandDataRecv(const ContactInfo &from, mesh::Packet *pkt, uint32_t sender_timestamp,
                               const char *text) {
  markConnectionActive(from); // in case this is from a server, and we have a connection
  queueMessage(from, TXT_TYPE_CLI_DATA, pkt, sender_timestamp, NULL, 0, text);
}

void CompanionMesh::onSignedMessageRecv(const ContactInfo &from, mesh::Packet *pkt, uint32_t sender_timestamp,
...
void CompanionMesh::onChannelMessageRecv(const mesh::GroupChannel &channel, mesh::Packet *pkt, uint32_t timestamp,
                                  const char *text) {
  int i = 0;
  if (app_target_ver >= 3) {
...
#ifdef DISPLAY_CLASS
    if (_ui) _ui->notify(CompanionUIEvent::channelMessage);
#endif
  }
#ifdef DISPLAY_CLASS
...
void CompanionMesh::onChannelDataRecv(const mesh::GroupChannel &channel, mesh::Packet *pkt, uint16_t data_type,
...
uint8_t CompanionMesh::onContactRequest(const ContactInfo &contact, uint32_t sender_timestamp, const uint8_t *data,
...
bool CompanionMesh::onContactPathRecv(ContactInfo& contact, uint8_t* in_path, uint8_t in_path_len, uint8_t* out_path, uint8_t out_path_len, uint8_t extra_type, uint8_t* extra, uint8_t extra_len) {
...
CompanionMesh::CompanionMesh(mesh::Radio &radio, mesh::RNG &rng, mesh::RTCClock &rtc, SimpleMeshTables &tables, CompanionStore& store, CompanionUITask* ui)
...
void CompanionMesh::begin(bool has_display) {
  BaseChatMesh::begin();
...
const char *CompanionMesh::getNodeName() {
  return _prefs.node_name;
}
CompanionNodePrefs *CompanionMesh::getNodePrefs() {
  return &_prefs;
}
uint32_t CompanionMesh::getBLEPin() {
...
bool CompanionMesh::isValidClientRepeatFreq(uint32_t f) const {
...
void CompanionMesh::handleCmdFrame(size_t len) {
  if (cmd_frame[0] == CMD_DEVICE_QUERY && len >= 2) { // sent when app establishes connection
    ...
    uint8_t *pub_key = &cmd_frame[2];
    CompanionAdvertPath* found = NULL;
...
void CompanionMesh::saveContacts() {
  _store->saveContacts(this, save_filter);
}

void CompanionMesh::enterCLIRescue() {
  _cli_rescue = true;
  cli_command[0] = 0;
  Serial.println("========= CLI Rescue =========");
}

void CompanionMesh::checkCLIRescueCmd() {
...
void CompanionMesh::checkSerialInterface() {
...
bool CompanionMesh::hasPendingWork() const {
