// GENERATED by tools/import_companion.py from MeshCore's examples/companion_radio/DataStore.cpp
...
CompanionStore::CompanionStore(FILESYSTEM& fs, mesh::RTCClock& clock) : _fs(&fs), _fsExtra(nullptr), _clock(&clock),
...
#if defined(EXTRAFS) || defined(QSPIFLASH)
CompanionStore::CompanionStore(FILESYSTEM& fs, FILESYSTEM& fsExtra, mesh::RTCClock& clock) : _fs(&fs), _fsExtra(&fsExtra), _clock(&clock),
#if defined(NRF52_PLATFORM) || defined(STM32_PLATFORM)
    identity_store(fs, "")
#elif defined(RP2040_PLATFORM)
    identity_store(fs, "/identity")
...
uint32_t CompanionStore::getStorageUsedKb() const {
#if defined(ESP32)
...
uint32_t CompanionStore::getStorageTotalKb() const {
#if defined(ESP32)
...
File CompanionStore::openRead(const char* filename) {
#if defined(NRF52_PLATFORM) || defined(STM32_PLATFORM)
...
File CompanionStore::openRead(FILESYSTEM* fs, const char* filename) {
#if defined(NRF52_PLATFORM) || defined(STM32_PLATFORM)
...
bool CompanionStore::removeFile(const char* filename) {
  return _fs->remove(filename);
}

bool CompanionStore::removeFile(FILESYSTEM* fs, const char* filename) {
  return fs->remove(filename);
}

bool CompanionStore::formatFileSystem() {
#if defined(NRF52_PLATFORM) || defined(STM32_PLATFORM)
  if (_fsExtra == nullptr) {
    return _fs->format();
  } else {
...
bool CompanionStore::loadMainIdentity(mesh::LocalIdentity &identity) {
  return identity_store.load("_main", identity);
}

bool CompanionStore::saveMainIdentity(const mesh::LocalIdentity &identity) {
  return identity_store.save("_main", identity);
}

void CompanionStore::loadPrefs(CompanionNodePrefs& prefs) {
...
void CompanionStore::loadPrefsInt(const char *filename, CompanionNodePrefs& _prefs) {
...
bool CompanionStore::savePrefs(CompanionNodePrefs& _prefs) {
...
void CompanionStore::loadContacts(CompanionStoreHost* host) {
...
void CompanionStore::saveContacts(CompanionStoreHost* host, bool (*filter)(const ContactInfo& c)) {
...
void CompanionStore::loadChannels(CompanionStoreHost* host) {
...
void CompanionStore::saveChannels(CompanionStoreHost* host) {
...
void CompanionStore::checkAdvBlobFile() {
...
void CompanionStore::migrateToSecondaryFS() {
...
uint8_t CompanionStore::getBlobByKey(const uint8_t key[], int key_len, uint8_t dest_buf[]) {
...
bool CompanionStore::putBlobByKey(const uint8_t key[], int key_len, const uint8_t src_buf[], uint8_t len) {
...
bool CompanionStore::deleteBlobByKey(const uint8_t key[], int key_len) {
