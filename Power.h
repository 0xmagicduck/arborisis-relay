...
float pmu_temperature = PMU_TEMP_MIN-1;
  ...
  // Half of VBAT on P0.31, behind a divider that P0.04 switches in. The
  // charger's state is not on any pin, so like the Heltecs the state is
  // read from the trend of the voltage.
  #define BAT_V_MIN       3.15
...
void measure_battery() {
  #if BOARD_MODEL == BOARD_RNODE_NG_21 || BOARD_MODEL == BOARD_LORA32_V2_1 || BOARD_MODEL == BOARD_HELTEC32_V3 || BOARD_MODEL == BOARD_HELTEC32_V4 || BOARD_MODEL == BOARD_TDECK || BOARD_MODEL == BOARD_T3S3 || BOARD_MODEL == BOARD_HELTEC_T114 || BOARD_MODEL == BOARD_TECHO || BOARD_MODEL == BOARD_WIO_TRACKER_L1
    ...
    #elif BOARD_MODEL == BOARD_WIO_TRACKER_L1
      // 10-bit read against the 3.6 V internal reference, through a 1:2
      // divider: 3.6 / 1024 * 2.
      float battery_measurement = (float)(analogRead(pin_vbat)) * 0.00703125;
    #else
...
bool init_pmu() {
    ...
    return true;
  #elif BOARD_MODEL == BOARD_HELTEC_T114 || BOARD_MODEL == BOARD_WIO_TRACKER_L1
    pinMode(pin_vbat, INPUT);
    pinMode(pin_ctrl,OUTPUT);
    digitalWrite(pin_ctrl, HIGH);
    return true;
